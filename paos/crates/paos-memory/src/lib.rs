...
pub fn ensure_schema(conn: &Connection) -> rusqlite::Result<()> {
    ...
    // Alternate phrasings lived here for one day and are gone. The idea was to store the
    // questions a fact answers so an abstract query could reach it, and it measured:
    // neutral on 70 questions single-stage (MRR 0.508 -> 0.523, three brains better and
    // three worse), and NEGATIVE once a second stage existed — 0.618 with against 0.614
    // without, while costing four questions at hit@5. A signal that does nothing still has
    // to be reasoned about by every future ranking change, so it was removed rather than
    // left switched on.
    //
    // The columns are dropped rather than abandoned: a dead column is a question the next
    // reader has to answer. `let _` because DROP COLUMN fails once they are already gone,
    // and this runs on every start.
    let _ = conn.execute("ALTER TABLE memories DROP COLUMN aliases", []);
    let _ = conn.execute("ALTER TABLE memories DROP COLUMN alias_embedding", []);
...
///
/// Diagnostic, not scoring. It answers whether a brain whose answers are present but badly
/// ordered is that way because the corpus is uniform — and on this store the answer was no,
/// the best-scoring brain being the most self-similar of all.
///
