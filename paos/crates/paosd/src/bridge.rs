...
fn session_topic(conn: &Connection, cfg: &Config, session: &str) -> Option<i64> {
    topic_for(conn, cfg, &own_room(conn, session))
}

/// The one room a session works in, or `ad-hocs` when that is not a single answer.
fn own_room(conn: &Connection, session: &str) -> String {
